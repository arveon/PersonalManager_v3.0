...
public class EditAppointmentWindow
{
	 ...
	 * @param toEdit 
	 */
	public EditAppointmentWindow(MainWindow window, Appointment toEdit, User user)
	{
		//this.user = user;
		this.toEdit = toEdit;
		this.list = window.getCurrentList();
		content = new JPanel();
		...
		int res = JOptionPane.showOptionDialog(null, content, "Personal Manager - Edit", JOptionPane.YES_NO_OPTION, JOptionPane.PLAIN_MESSAGE, null, options, options[1]);
		if(res == 1)
		{
			...
			int msg = list.editAppointment(toEdit, new Appointment(descArea.getText(), (String)placeField.getText(), y, m, d, h, min, user.getName()));
			JOptionPane.showMessageDialog(null, AppointmentList.errors[msg]);
			if(msg == 4)
			{
				window.setList(list);
				window.updateTable(list);
			}
		}
	}
	...
	public void initializeTimeBoxes()
	{
		...
		month.addItemListener(new ItemListener(){
			...
			public void itemStateChanged(ItemEvent event)
			{
				...
				//get the month combobox and pull out the month number selected from it
				JComboBox<String> month = (JComboBox<String>)event.getSource();
				String str = (String)month.getSelectedItem();
				int m = Integer.parseInt(str);
				
				//set the number variable depending on the month
				int number = 31;
				...
				//set the new values of the day combobox
